...
namespace SDK {
    bool C_Human2CarWrapper::IsDriver(C_Actor *pActor) const {
        return GetSeatID(pActor) == 0;
    }

    bool C_Human2CarWrapper::IsEngineOn() const {
        return hook::this_call<bool>(gPatterns.C_Human2CarWrapper__IsEngineOn, this);
    }

    unsigned int C_Human2CarWrapper::GetSeatID(C_Actor *pActor) const {
